...
BPF_TABLE("hash", u64, int, obj_stack, 100000);
...
/*
It would be annoying to always have to instrument both entry and return, 
just to fetch the stack on entry then save it by tid so we can read it on return.
*/
static inline int kp_alloc_core(struct pt_regs *ctx)
{
    unsigned char *kc;
    unsigned int size = 0;
    int stack_id;
    ...
    nested_level.increment(zero);

    stack_id = stack_traces.get_stackid(ctx, 0);

    idx = nested_level.lookup(&zero);
    if (idx){
        //bpf_trace_printk("Found %d %d\\n", size, *idx);
        d.stack_id = stack_id;
        nested_stacks.update(idx, &d);
        return 1;
    }
...
static inline int kretp_alloc_core(struct pt_regs *ctx)
{
    int *idx = 0, v, id;
    u32 zero = 0;
    u64 obj;
    ...
    if (idx) {
        id = *idx;
        v = id - 1;
        d = nested_stacks.lookup(&id);
        if (d == 0) {
//            nested_level.update(&zero, &v);
            return 0;
        }
        bpf_probe_read_kernel(&obj, sizeof(obj), &ctx->ax);

        obj_stack.update(&obj, &d->stack_id);
        nested_level.update(&zero, &v);
...
{
        int *stack_id = obj_stack.lookup(&obj);
        if (stack_id == 0)
                return 0;
...
int kp_put_cred_rcu(struct pt_regs *ctx) {
    u64 obj;
    
    bpf_probe_read_kernel(&obj, sizeof(obj), &ctx->di);
    //   [0x98] struct callback_head rcu;
    return kp_free_core(ctx, obj - 0x98);
}

int kp_prepare_kernel_cred(struct pt_regs *ctx) {

    return kp_alloc_core(ctx);
}

int kretp_prepare_kernel_cred(struct pt_regs *ctx) {
    return kretp_alloc_core(ctx);
}

int kp_prepare_creds(struct pt_regs *ctx) {

    return kp_alloc_core(ctx);
}

int kretp_prepare_creds(struct pt_regs *ctx) {
    return kretp_alloc_core(ctx);
}


int kp_cred_alloc_blank(struct pt_regs *ctx) {

    return kp_alloc_core(ctx);
}

int kretp_cred_alloc_blank(struct pt_regs *ctx) {
    return kretp_alloc_core(ctx);
}
...
bpf.attach_kretprobe(event="prepare_kernel_cred", fn_name="kretp_prepare_kernel_cred")
bpf.attach_kprobe(event="prepare_kernel_cred", fn_name="kp_prepare_kernel_cred")
bpf.attach_kretprobe(event="prepare_creds", fn_name="kretp_prepare_creds")
bpf.attach_kprobe(event="prepare_creds", fn_name="kp_prepare_creds")
bpf.attach_kretprobe(event="cred_alloc_blank", fn_name="kretp_cred_alloc_blank")
bpf.attach_kprobe(event="cred_alloc_blank", fn_name="kp_cred_alloc_blank")
bpf.attach_kprobe(event="put_cred_rcu", fn_name="kp_put_cred_rcu")
...
def print_outstanding_x():
        print("\n [%s] Top %d stacks with outstanding cred_jar allocations:" %
              (datetime.now().strftime("%H:%M:%S"), top_stacks))
        allocs = bpf["obj_stack"]
        stack_counts = {}
        for obj, stack_id in allocs.items():
            id = stack_id.value
            #print("stack_id  %d %lx\n"%(id, obj.value))
            if id not in stack_counts:
                stack_counts[id] = 0
            stack_counts[id] = stack_counts[id] + 1 

        i = 0
        for stack_id, counts in sorted(stack_counts.items(), key=lambda a: a[1]):
                if stack_id < 0:
                        continue
                print("stack_id %d, outstanding memory %d bytes:"%(stack_id, counts * 0xc0))
                dst_x(stack_id)
                i = i + 1
                if i >= top_stacks:
                    break
