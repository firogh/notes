...
static inline int kp_alloc_core(struct pt_regs *ctx, int len)
{
    unsigned char *kc;
...
//        bpf_trace_printk("Found ret %d %lx\\n", d->stack_id, obj);

        //bpf_trace_printk("Found ret %d %d\\n", d->size, *idx);

        // update_statistics_add(d->stack_id, d->size);
        nested_level.update(&zero, &v);
...
def print_outstanding_x():
        print("\n [%s] Top %d stacks with outstanding kmalloc-512 allocations:" %
              (datetime.now().strftime("%H:%M:%S"), top_stacks))
        ...
        for stack_id, counts in sorted(stack_counts.items(), key=lambda a: a[1], reverse=True):
                if stack_id < 0:
                        continue
                print("stack_id %d, outstanding memory %d bytes:"%(stack_id, counts * 512 ))
                dst_x(stack_id)
                i = i + 1
                if i >= top_stacks:
                    break
